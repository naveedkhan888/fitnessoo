...
class Xpoint_Theme_Updater {
	...
	public function add_license_invalid_notice() {
		...
		if ( ! $license || $status !== 'valid' ) {
			...
			\Xpoint_Admin_Notice_Manager::instance()->warning( $args );
		}
	}
		...
		if ( $this->is_theme_license_page() && isset( $_GET['success'] ) && ! empty( $_GET['message'] ) ) {
			...
			switch ( $_GET['success'] ) {
				case 'yes':
					...
					\Xpoint_Admin_Notice_Manager::instance()->success( $args );
					break;
				case 'no':
					$args['notice_id'] = 'error_license';
					\Xpoint_Admin_Notice_Manager::instance()->error( $args );
					break;
				default:
