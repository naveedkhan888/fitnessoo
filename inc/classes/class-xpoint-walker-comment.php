 ...
 */
class Xpoint_Walker_Comment extends Walker_Comment {
	...
	protected function ping( $comment, $depth, $args ) {
		...
		<<?php echo esc_attr( $tag ); ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( '', $comment ); ?>>
			<div class="comment-body">
				<?php
				esc_html_e( 'Pingback:', 'themename' );
				echo esc_html( '&nbsp;' );
				?>
				<?php comment_author_link( $comment ); ?> <?php edit_comment_link( esc_html__( 'Edit', 'themename' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
		<?php
	}
							...
							<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
								<time datetime="<?php comment_time( 'c' ); ?>">
									<?php
										...
										printf( esc_html__( '%1$s at %2$s', 'themename' ), get_comment_date( '', $comment ), get_comment_time() );
									?>
								</time>
							</a>
