 ...
 */
if ( ! function_exists( 'xpoint_the_lazy_image' ) ) {
	function xpoint_the_lazy_image( $args ) {
		$defaults = array(
			'id'        => null,
			'type'      => 'background',
			'size'      => 'full',
		...
		$lazy_placeholder_src = apply_filters( 'xpoint/lazy/placeholder', $lazy_placeholder_src );
		...
		if ( $args['type'] === 'background' || $args['type'] === 'image' ) {
			...
			$enable_optimized_sizes   = apply_filters( 'xpoint/lazy/enable_optimized_sizes', ! $full_size_images_enabled );
