			...
			<?php if ( ! $is_theme_registered ) : ?>
				<form class="merlin__content--license-key" action="<?php echo esc_url( $action_url ); ?>" method="POST" autocomplete="off">
					...
					<div class="merlin__content--license-key-wrapper">
						<input type="text" id="license-key" class="js-license-key" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" maxlength="36" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" pattern="[0-9a-zA-Z]{8}-[0-9a-zA-Z]{4}-[0-9a-zA-F]{4}-[0-9a-zA-Z]{4}-[0-9a-zA-Z]{12}" required title="Purchase code format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx">
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				...
				if ( 'valid' === $license_data->license ) {
					...
					update_option( $this->edd_theme_slug . '_license_key', sanitize_text_field( $license ) );
...
";
	...
	function _ajax_plugins() {
		...
		if ( $json ) {
			...
			$json['hash']    = hash( 'sha256', serialize( $json ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['hash']     = hash( 'sha256', rand() ); // Has to be unique (check JS code catching this AJAX response).
